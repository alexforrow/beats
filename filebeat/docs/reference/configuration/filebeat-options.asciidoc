...
(or other key if `target` is specified). If you enable this setting, the keys are copied top level in 
the output document. The default is false.
...
*`target`*:: An optional configuration setting that specifies the name of the JSON key under which 
the decoded JSON is placed under (when `keys_under_root` is not specified). Default is "json"
